...
- `bind`：负责把 query / body 绑定到 Go 目标值
...
- `bind.BindQuery(...)` / `bind.BindBody(...)` 是 DTO 场景下的补充能力
...
| 批量 query DTO 绑定 | `hah.BindQuery` / `bind.BindQuery` | 适合筛选条件、分页参数、复杂多值 query |
| 只做 JSON body 绑定 | `hah.BindBody` / `bind.BindBody` | 适合 body DTO 解码 |
...
- `Query` 额外支持 `Values()` / `Strings()` 读取重复 query 的原始值
...
`bind.BindQuery` / `hah.BindQuery` 只从 query 参数绑定数据。
...
它适合“批量投影”，不适合表达请求级规则。像 `Required`、`Default`、`OneOf`、`Min/Max` 这类规则，仍然优先放在 `reqx.Query(...)` 或 bind 后的显式校验里。

### body DTO 绑定

`bind.BindBody` / `hah.BindBody` 只从请求 body 绑定数据。
