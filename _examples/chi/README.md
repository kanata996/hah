...
6. 当更深层已经明确要暴露稳定公共 HTTP 错误时，可以直接返回 `hah` 公共错误；失败路径统一走 `hah.WriteError(...)`。
7. 成功路径通常走 `hah.OK(...)` / `hah.Accepted(...)` / `hah.Created(...)`；若调用方明确需要无响应体成功，也可以显式用 `hah.NoContent(...)`。当前示例的删除成功仍返回 envelope。
