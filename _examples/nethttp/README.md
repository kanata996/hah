...
- 只使用当前根包公开 API：`Bind`、`RequireBody`、`OK`、`Created`、`NoContent`、`WriteError`
- path/query/body 都走 `hah` 的 `net/http` 绑定契约，不依赖 router 私有上下文
- handler 在 `Bind(...)` 之后显式做最小输入校验
