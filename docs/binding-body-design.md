...
支持范围是一个闭合但受限的 family：终端类型、`time.Time`、`struct` 家族、`[]T` 家族与一级指针。
...
- 类型家族按字段形状判断，而不是按是否命名判断；因此命名 `struct` 类型属于 `struct` 家族，底层为 `[]T` 的命名类型属于 `[]T` 家族
- 对受支持的 `struct` 字段，递归后的所有参与 JSON binding 的子字段也必须继续落在本文支持范围内，否则属于 usage error
- 对受支持的 `[]T` 字段，元素类型 `T` 也必须继续落在本文支持范围内，否则属于 usage error
...
| `struct` 家族                                                                | 是       | 递归按相同规则解码其参与 JSON binding 的字段            | 包括命名 `struct` 类型；子字段继续受未知字段拒绝与重复 key 检查 |
| `[]T` 家族                                                                   | 是       | 按标准库 slice 语义解码                                 | 包括底层为 `[]T` 的命名类型；`T` 只允许是表内支持的终端类型、`time.Time` 或 `struct` 家族 |
...
- `[]T` 的代表性成功 / 失败路径
- 命名 `struct` 类型与底层为 `[]T` 的命名类型，分别按对应家族规则支持
